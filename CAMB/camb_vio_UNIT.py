...
# of UNIT simulation to run SAMs (galform and shark) over this cosmology

#omega0, omegab, lambda0, h0, volume = b.get_cosmology(sim,envs[iis])
cosmo = 'UNIT'
...
#minim = np.amin(np.abs(k-kout[0]))
#ind = np.where(np.abs(k-kout[0])==minim)
#here = ind[0][0]
#print(here)
#k = k[here:here+len(kout)]
#Pk = Pk[here:here+len(kout)]
#Pout_lin = PKlin.P(z,k)

#for i in range(nk):
#    print(k[i],Pout_lin[i]*snorm)#,Pout_NL[i],Pout_NL[i]*snorm)
#    print(k[i],Pk[i])#,Pout_NL[i],Pout_NL[i]*snorm)
